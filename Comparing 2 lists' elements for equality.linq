...
// Would have to use a class implementing the IEqualityComparer interface in order for these to be equal.
scheduleSettings.SequenceEqual(dbSchedList).Dump();

// Or could maybe use this instead if order doesn't matter
var a = new HashSet<AutoReportScheduleSettings>(scheduleSettings).SetEquals(dbSchedList);
a.Dump();