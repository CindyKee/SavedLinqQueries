...
//		.Take(10)
	...
	var groupedBy = queryable
		...
		.Select(g => new PhaseGroupingForReport
		{
			OrgID = g.Key.Org.ID,
	...
	Console.WriteLine("Grouped by Org and Phase:");
	groupedBy.Count().Dump();
	groupedBy.Dump();

	// Apply group by parameters here (Phase, Phase IN, and ProjectsCount)
	Console.WriteLine("Grouped by results filtered by Phase=Startup:");
	groupedBy = groupedBy.Where(data => data.Phase.Equals("Startup"));
	groupedBy.Count().Dump();
	groupedBy.Dump();
}
